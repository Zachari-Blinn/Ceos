...
class MatiereController extends AbstractController
{
     ...
     */
    public function delete(Request $request, Matiere $matiere, EnseignementRepository $enseignementRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$matiere->getId(), $request->request->get('_token')))
        {
            $enseignements = $enseignementRepository->findBy(['matiere' => $matiere->getId()]);

            if($enseignements != null)
            {
                $this->addFlash('danger', 'Erreur - suppression impossible : La matière possède un ou des enseignement(s) !');
            }
            else
            {
                $entityManager = $this->getDoctrine()->getManager();
