...
class ProfController extends AbstractController
{
     ...
     */
    public function delete(Request $request, Prof $prof, EnseignementRepository $enseignementRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$prof->getId(), $request->request->get('_token')))
        {
            $enseignements = $enseignementRepository->findBy(['prof' => $prof->getId()]);

            if($enseignements != null)
            {
                $this->addFlash('danger', 'Erreur - suppression impossible : Le professeur possède un ou des enseignement(s) !');
            }
            else
            {
                $entityManager = $this->getDoctrine()->getManager();
